...
pubBtn.addEventListener('hover', function pubBtnExpand() {
    if (isHovering) {
        scale += 0.2;
        pubBtn.style.transform = `scale(${scale})`;

        animationFrameId = requestAnimationFrame(pubBtnExpand);
    };
});

pubBtn.addEventListener('mouseenter', () => {
    isHovered = true;
    cancelAnimationFrame(animationFrameId);
    requestAnimationFrame(pubBtnExpand());
});

pubBtn.addEventListener('mouseleave', () => {
    isHovered = false;
    cancelAnimationFrame(animationFrameId);
});